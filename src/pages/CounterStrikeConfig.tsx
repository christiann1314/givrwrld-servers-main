...
const csBackdrop = 'https://cdn.akamai.steamstatic.com/steam/apps/730/library_hero.jpg';

const CounterStrikeConfig = () => {
  const navigate = useNavigate();
  const { user } = useAuth();
  const [serverName, setServerName] = useState('');
  const [region] = useState('us-east');
  const [planId, setPlanId] = useState(CATALOG_STARTERS['counter-strike'].defaultPlanId);
  const [gameType, setGameType] = useState('counter-strike-standard');
  ...
  const { run: createCheckout, loading } = useAction(async () => {
    ...
    const response = await stripeService.createCheckoutSession({
      ...
      cancel_url: `${window.location.origin}/configure/counter-strike`,
    });

    window.location.href = response.checkout_url;
  });

  const fallbackPlans = CATALOG_STARTERS['counter-strike'].plans;
  const fallbackGameTypes = CATALOG_STARTERS['counter-strike'].gameTypes;

  const { plans, gameTypes, getPriceForTerm } = useGamePlanCatalog('counter-strike', fallbackPlans, fallbackGameTypes);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white relative overflow-hidden">
      <div
        ...
        style={{
          backgroundImage: `url(${csBackdrop})`,
          backgroundSize: 'cover',
          backgroundPosition: 'center center',
          backgroundAttachment: 'fixed',
        }}
      >
        <div className="absolute inset-0 bg-gradient-to-b from-gray-900/55 via-gray-900/40 to-gray-900/60"></div>
      </div>
      ...
      <div className="relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <p className="text-lg text-gray-100 max-w-3xl mb-8">
            Classic SRCDS dedicated hosting — set hostname, map, and workshop options from the panel after deploy.
          </p>

          <div className="bg-amber-600 text-white px-6 py-3 rounded-lg mb-8 inline-block">
            Linux dedicated (App 740) · Add a Steam GSLT in the panel for public listing
          </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-gray-800/60 backdrop-blur-md border border-gray-600/50 rounded-xl p-6">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-white font-semibold mb-2">Hostname (server name)</label>
                    <input
                      type="text"
                      value={serverName}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {billingTerms.map((term) => (
                    ...
                    >
                      ...
                      {term.id === 'semiannual' ? (
                        <div className="text-xs text-amber-200 font-medium">Best value · Save {term.discount}%</div>
                      ) : term.discount > 0 ? (
          ...
          <GameTransparencySection gameSlug="counter-strike" accentColor="amber" />
        </div>
      </div>
    </div>
  );
...
export default CounterStrikeConfig;
