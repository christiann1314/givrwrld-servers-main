  ...
  React.useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!active) return;
        setError(err?.message || "Unable to load streamer directory.");
      } finally {
          ...
          {loading ? (
            ...
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {streamers.map((streamer) => {
                const statusClassName = statusTone[String(streamer.status || "unknown").toLowerCase()] || statusTone.unknown;
                const title = streamer.streamer_name || streamer.server_name || streamer.game || "GIVRwrld Streamer";
                const watchLabel =
